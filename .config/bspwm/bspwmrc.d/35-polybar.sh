...
if /usr/bin/pgrep -x polybar >/dev/null; then
    ...
    while /usr/bin/pgrep -x polybar >/dev/null; do /usr/bin/sleep 1; done
fi
...
for m in $(/usr/bin/xrandr --query | /usr/bin/grep " connected" | /usr/bin/cut -d" " -f1); do
    ...
    MONITOR="$m" \
        ...
        FONT2="$_FONT2" \
        FONT3="$_FONT3" \
        /usr/bin/polybar --reload launcher >$XDG_DATA_HOME/polybar/launcher.log 2>&1 & disown
    ...
    MONITOR="$m" \
        ...
        FONT2="$_FONT2" \
        FONT3="$_FONT3" \
        /usr/bin/polybar --reload workspaces >$XDG_DATA_HOME/polybar/workspaces.log 2>&1 & disown
    ...
    MONITOR="$m" \
        ...
        FONT2="$_FONT2" \
        FONT3="$_FONT3" \
        /usr/bin/polybar --reload modules0 >$XDG_DATA_HOME/polybar/modules0.log 2>&1 & disown
    ...
    if [ "$_VIEWPORT_WIDTH" = "1920" ]; then
        MONITOR="$m" \
            ...
            /usr/bin/polybar --reload modules1 >$XDG_DATA_HOME/polybar/modules1.log 2>&1 & disown
    fi
    ...
    MONITOR="$m" \
        ...
        FONT2="$_FONT2" \
        FONT3="$_FONT3" \
        /usr/bin/polybar --reload timestamp >$XDG_DATA_HOME/polybar/timestamp.log 2>&1 & disown
    ...
    MONITOR="$m" \
        ...
        FONT2="$_FONT2" \
        FONT3="$_FONT3" \
        /usr/bin/polybar --reload power >$XDG_DATA_HOME/polybar/power.log 2>&1 & disown
done
